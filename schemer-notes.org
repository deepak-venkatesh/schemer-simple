...
+ Any function name with a ~*~ at the end basically means application for the entire list. We write ~rember*~ which removes a specific element from even nested lists.
  The logic is to simply check is the element in the list is an atom and equal to the removable element, if it is then we just do a recursive ~rember*~ on the ~cdr~ of the list. If it is not equal
